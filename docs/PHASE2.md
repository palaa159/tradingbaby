...
## ปิดช่วงที่ 2 (2026-07-29)

ตอนที่คอมมิต P7 เขียนว่า "Phase 2 complete" มันจริงในแง่ว่า*โค้ดครบ* แต่ไม่จริง
ในแง่ว่า*ทำงานจริง* — ตรวจย้อนทีหลังเจอสามช่องโหว่ ที่ทั้งสามเป็นแบบเดียวกัน:
ของถูกสร้างไว้แล้ว แต่ไม่มีใครเรียกใช้ หรือเรียกแล้วผลหายไป

**เครื่องรันสูตรไม่เคยเดิน** `paperTrader.tick()` ไม่มีคนเรียกนอกจากเทสต์ของตัวเอง
และคลาส `Metabolism` ก็เหมือนกัน แปลว่ากำไรไม่เคยกลายเป็นพลังงานสักครั้ง
ตอนนี้มี `alpha-trader` เป็น process แยก เดินตามนาฬิกาแท่งเทียน ไม่กิน AI เลย
(ข้อ 6.2, ข้อ 10) — แยกจาก daemon เพราะการคิดถูกจำกัดด้วยโควตา แต่การเทรดไม่ใช่
นักเรียนที่พลังงานหมดจึงยังหากินจากกฎที่ตัวเองพิสูจน์แล้วได้

**สมุดพกถูกพิมพ์ทิ้ง** `buildReportCard` พิมพ์ลง terminal แล้วจบ เกรดของสถาบันเอง
เลยเป็นตัวเลขเดียวที่ย้อนดูไม่ได้ ตอนนี้เก็บทั้งใบและคำตอบทุกข้อที่อยู่เบื้องหลัง

**กล่องคำร้องมีปลายเดียว** นักเรียนเขียน `feature_request` ได้ ครูใหญ่นับได้
แต่คนสร้างอ่านไม่ได้ — ซึ่งขัดกับข้อ 12 ที่บอกว่าคำร้องกองรอให้คนสร้างทำเอง

**วงจรหลักไม่เคยครบสักรอบ** ของจริงมี hypothesis 0 · strategy 0 · evaluation 0
และเทสต์เดิมครอบคลุมแต่ทาง*ปฏิเสธ* (สเปคพัง, หลักฐานไม่พอ, นักเรียนอ้างเอง)
ไม่มีที่ไหนยืนยันว่าเกิดอะไรขึ้นเมื่อหลักฐาน*ผ่าน* ตอนนี้ยืนยันครบสาย:
ข้อสงสัย → ทดสอบย้อนหลัง → กรรมการรับ → ความมั่นใจขึ้น → สูตร v1 ที่แก้ไม่ได้ →
ไม้จริง → รันซ้ำได้ผลตรงเป๊ะ → สาวกลับไปถึงข้อสงสัยที่เป็นต้นเหตุ

เทสต์นั้นใช้ข้อมูลจำลอง ไม่ใช่ BTC จริง โดยตั้งใจ: ลองกับตลาดจริงก่อนแล้วได้
"ปฏิเสธ" (เทรด 3 ครั้ง ต้องการ 5) แล้วได้ "ตีตก" (alpha -3.04%) ทั้งคู่ถูกต้อง —
ช่วง 400 แท่งนั้น BTC ขึ้น 3.20% การนอนถือเฉยๆ ชนะทุกกฎที่ลอง
กฎจะชนะการนอนถือไหมเป็นเรื่องของตลาด แต่คำตัดสินที่ผ่านแล้วจะเดินทางไปถึงไม้ที่
รันซ้ำได้ไหมเป็นเรื่องของโค้ด และมีแค่อย่างหลังที่ควรตัดสินว่าเทสต์เขียวหรือแดง

สิ่งที่ตามผิดทางไปหนึ่งรอบ: เห็นไม้เข้าที่ราคา 107 ตอน rsi < 35 แล้วนึกว่าสัญญาณกลับด้าน
ไม่ใช่ — `rsi` คืน 100 กับขาขึ้นล้วน คืน 0 กับขาลงล้วน `sma(5)` ตรงเป๊ะ และดัชนี
ของ series ตรงกับ input ทุกตัว RSI วัด*โมเมนตัม* ไม่ใช่*ระดับราคา* 107 ที่กำลังไหลลง
คือ oversold จริงๆ กฎมันแค่แย่บนชุดข้อมูลนั้น
